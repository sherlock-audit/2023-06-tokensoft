...
contract SaleManager_v_1_3 is ReentrancyGuard, PullPayment, ISaleManager_v_1_3 {
	...
	struct Sale {
		...
		uint256 startTime; // the time at which the sale starts (seconds past the epoch)
		uint256 endTime; // the time at which the sale will end, regardless of tokens raised (seconds past the epoch)
	...
	modifier validSale(bytes32 saleId) {
		...
		require(sales[saleId].admin != address(0), "invalid sale id");
		_;
	}

	modifier isAdmin(bytes32 saleId) {
		// msg.sender is never address(0) so this handles uninitialized sales
		require(sales[saleId].admin == msg.sender, "must be admin");
		_;
	}
	...
	// Accessor functions
	function getAdmin(bytes32 saleId) public view validSale(saleId) returns (address) {
		return (sales[saleId].admin);
	}

	function getRecipient(bytes32 saleId) public view validSale(saleId) returns (address) {
	...
	function getPurchaseMinimum(bytes32 saleId) public view validSale(saleId) returns (uint256) {
	...
	function getUri(bytes32 saleId) public view validSale(saleId) returns (string memory) {
  ...
  */
	...
	function setStart(bytes32 saleId, uint256 startTime) public validSale(saleId) isAdmin(saleId) {
	...
	function setEnd(bytes32 saleId, uint256 endTime) public validSale(saleId) isAdmin(saleId) {
	...
	function setUriAndMerkleRoot(
		bytes32 saleId,
		bytes32 merkleRoot,
		string calldata uri
	) public validSale(saleId) isAdmin(saleId) {
	...
	function buy(
		...
		require(tokenQuantity >= sales[saleId].purchaseMinimum, "purchase below minimum");
